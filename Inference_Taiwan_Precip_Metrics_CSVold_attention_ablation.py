...
# -*- coding: utf-8 -*-
"""
Inference_Taiwan_Precip_Metrics_CSVold_attention_ablation.py
...
Purpose:
  Run the author-style Taiwan precipitation EDM inference for either a model
  trained WITH self-attention or a separately trained CNN-only model.

Important:
  The architecture flags used here MUST match the checkpoint used at training.
  load_state_dict(strict=True) is intentionally retained so that accidentally
  loading an Attention checkpoint into a No-Attention model (or vice versa)
  fails immediately instead of silently producing an invalid comparison.

8-GPU example (No-Attention model):
  torchrun --standalone --nproc_per_node=8 \
    Inference_Taiwan_Precip_Metrics_CSVold_attention_ablation.py \
    --checkpoint ./outputs_taiwan_8km_bilinear_noattn_150/Model/149.pt \
    ...
    --dropout 0.10 \
    --no-attention \
    --output-csv ./outputs_taiwan_8km_bilinear_noattn_150/precip_metrics_per_date.csv \
    --summary-csv ./outputs_taiwan_8km_bilinear_noattn_150/precip_metrics_summary.csv
"""
...
# Modified: use the attention-ablation-capable version of Network.py.
...
def resolve_checkpoint_path(value: str) -> Path:
    """Resolve checkpoint file or choose highest numeric epoch in Model/."""
...
def load_author_checkpoint(
    ...
    # Original author-style trainer saves a pure state_dict.
    # This fallback also accepts a wrapped dictionary if supplied.
...
# -----------------------------------------------------------------------------
# Author-style EDM / second-order Heun sampling
...
def metric_batch(
    ...
    ensemble_predictions: [M, B, 1, H, W]
    MAE/RMSE are computed on the ensemble-mean prediction.
    """
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Author-style Taiwan bilinear diffusion inference with optional "
            "self-attention architecture; write land-only MAE/RMSE to CSV."
        )
    )
    ...
    attention_group.add_argument(
        ...
        dest="use_attention",
        action="store_true",
        help="Checkpoint was trained with self-attention enabled.",
    )
    attention_group.add_argument(
        "--no-attention",
        dest="use_attention",
        action="store_false",
        help="Checkpoint was trained from scratch with self-attention disabled.",
    )
...
def main() -> None:
    ...
    # Modified: reconstruct exactly the same ON/OFF architecture used in training.
    ...
    try:
        ...
        raise RuntimeError(
            "Checkpoint/model architecture mismatch. Confirm that "
            "--use-attention/--no-attention and all architecture options "
            "match the training run exactly. "
            f"Current inference architecture: use_attention={args.use_attention}, "
            f"model_channels={args.model_channels}, channel_mult={channel_mult}, "
            f"num_blocks={args.num_blocks}, attn_resolutions={attn_resolutions}, "
            f"dropout={args.dropout}. Original error: {exc}"
        ) from exc
    ...
    if is_main_process():
        ...
        common = {
            ...
            "model_channels": args.model_channels,
            "channel_mult": ",".join(str(x) for x in channel_mult),
            "num_blocks": args.num_blocks,
            "attn_resolutions": ",".join(str(x) for x in attn_resolutions),
